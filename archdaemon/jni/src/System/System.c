    ...
    while (1) {
        ...
        if (ctx.need_profile_checkup) {
            ...
            if (current_focused_game) {
                ...
                /* Cek apakah game yang sedang aktif masih berjalan di background / recent apps */
                pid_t check_pids[MAX_GAME_PIDS];
                int active_count = get_pids_of(gamestart, check_pids, MAX_GAME_PIDS);
                if (active_count == 0) {
                    log_zenith(LOG_INFO, "Game %s is closed/removed from recent apps, returning to Balanced Profile",
                    ...
                    /* Game di-minimize tapi masih hidup di recent apps / background. Pertahankan Performance Profile */
                    log_zenith(LOG_INFO, "Game %s minimized in background (%d PID(s)), keeping Performance Profile",
                               active_app_name ? active_app_name : gamestart, active_count);
                    ctx.need_profile_checkup = false;
                }
        ...
        if (ctx.is_initialize_complete && gamestart && effective_screen_state) {
            ...
            if (game_pid_count == 0) [[clang::unlikely]] {
                ...
                if (game_pid_count == 0 && strcmp(current_system_cache.focused_app, gamestart) == 0 && current_system_cache.focused_pid > 0) {
