#!/system/bin/sh
# AZenith RN9 fork - fstrim terjadwal async setelah boot.
#
# Boot-time fstrim dihapus di fork ini karena bikin ANR. Sebagai gantinya,
# trim dijalankan di background beberapa menit SETELAH boot (idle), lalu
# diulang berkala. Ini menjaga performa tulis eMMC tanpa nge-freeze UI.
#
# Dipanggil service.sh saat boot: azenith-fstrim.sh watch &
# Manual / debug                : azenith-fstrim.sh once | status
#
# Config (/data/adb/.config/AZenith/maint/):
