...
#
# Dipanggil daemon  : azenith-hibernate.sh apply | release
# Fallback watcher  : azenith-hibernate.sh watch   (dijalankan service.sh)
# Manual / debug    : azenith-hibernate.sh status | boot-reset | apply --force
#
...
LEGACY_ECO_DIR=$MODULE_CONFIG/hibernate
LIST=$ECO_DIR/hibernate.list
LOG=$ECO_DIR/hibernate.log
STATE=$ECO_DIR/.applied
ENABLED=$ECO_DIR/enabled
DELAY_FILE=$ECO_DIR/delay
# Per-app + kondisi (fitur RN9):
MODE_DEFAULT_FILE=$ECO_DIR/mode.default   # full | restrict
MODE_LIST=$ECO_DIR/mode.list              # baris: "paket mode"
SKIP_CHARGING_FILE=$ECO_DIR/skip_charging # 1 = jangan hibernasi saat ngecas
SKIP_AUDIO_FILE=$ECO_DIR/skip_audio       # 1 = jangan hibernasi saat audio jalan
WATCH_LOCK=/dev/.azenith_hibernate.lock
MAXLOG=131072

# Direktori state WAJIB ada, kalau tidak semua tulisan log/state gagal diam-diam.
mkdir -p "$ECO_DIR" 2>/dev/null

# Migrasi dari fork RN9 versi pertama.
if [ ! -f "$LIST" ] && [ -f "$LEGACY_ECO_DIR/hibernate.list" ]; then
	cp -f "$LEGACY_ECO_DIR/hibernate.list" "$LIST" 2>/dev/null
fi
[ -f "$LIST" ] || : >"$LIST"
...
# Mode hibernasi default: full (force-stop) atau restrict (batasi tanpa bunuh).
mode_default() {
...
# Mode per-app dari mode.list (format: "paket mode"). Fallback ke mode_default.
app_mode() {
	_p=$1
	if [ -f "$MODE_LIST" ]; then
...
# Ada pemutaran audio aktif (mis. musik saat layar mati)?
audio_active() {
...
# Baca daftar user, abaikan baris kosong dan komentar.
pkglist() {
	[ -f "$LIST" ] || return 0
	sed -e 's/#.*//' -e 's/[[:space:]]//g' "$LIST" | grep -v '^$'
}

# pm bisa gagal total kalau dipanggil dari konteks daemon. Kalau perintahnya
# memang tidak tersedia, jangan anggap semua app "not installed" (bug lama).
is_installed() {
...
is_protected() {
	case "$1" in
	android | com.android.systemui | com.android.phone | com.android.settings | zx.azenith | *.inputmethod* | *inputmethod* | com.google.android.gms | com.google.android.gsf | com.android.vending)
		return 0
		;;
	esac
	return 1
...
# freeze_full = Baterai "Dibatasi" (Restricted) + matikan proses (force-stop).
#   Hemat maksimal, tapi notifikasi app TIDAK masuk selama hibernasi.
#   Restricted ditentukan oleh appop RUN_ANY_IN_BACKGROUND = ignore.
freeze_full() {
	p=$1
	# Kunci status "Dibatasi": RUN_ANY_IN_BACKGROUND WAJIB ignore.
	appops set "$p" RUN_ANY_IN_BACKGROUND ignore >/dev/null 2>&1
	# Op pendukung supaya benar-benar tidak ada aktivitas background.
	...
	# Keluarkan dari whitelist Doze (Restricted tidak boleh di-exempt).
	cmd deviceidle whitelist -"$p" >/dev/null 2>&1
	am force-stop "$p" >/dev/null 2>&1
}

# freeze_restrict = Baterai "Dibatasi" (Restricted) TANPA force-stop.
#   Background dibatasi (hemat), tapi app tetap hidup sehingga notifikasi
#   penting (mis. FCM via GMS) masih bisa masuk. Cocok untuk app chat.
freeze_restrict() {
	p=$1
	appops set "$p" RUN_ANY_IN_BACKGROUND ignore >/dev/null 2>&1
...
# freeze = dispatcher sesuai mode per-app (full / restrict).
freeze() {
	case "$(app_mode "$1")" in
...
# thaw = kembalikan aplikasi ke Baterai "Dioptimalkan" (Optimized).
#   Optimized = RUN_ANY_IN_BACKGROUND diizinkan TAPI TIDAK ada di whitelist Doze.
#   (Kalau ikut di-whitelist, statusnya jadi "Tak dibatasi", bukan "Dioptimalkan".)
thaw() {
	p=$1
	# Kunci status "Dioptimalkan": RUN_ANY_IN_BACKGROUND WAJIB allow.
	appops set "$p" RUN_ANY_IN_BACKGROUND allow >/dev/null 2>&1
	# Op pendukung dikembalikan ke bawaan sistem.
	for op in RUN_IN_BACKGROUND WAKE_LOCK START_FOREGROUND; do
		appops set "$p" $op default >/dev/null 2>&1
	done
	am set-standby-bucket "$p" active >/dev/null 2>&1
	# Pastikan TETAP di luar whitelist Doze supaya berakhir di "Dioptimalkan".
	cmd deviceidle whitelist -"$p" >/dev/null 2>&1
}
...
do_apply() {
	...
	# Kondisi user: tunda hibernasi saat ngecas / ada audio aktif (kecuali --force).
	if [ "$1" != "--force" ]; then
...
release)
	...
	;;
boot-reset)
	# Setelah reboot, state lama bikin 'apply' langsung exit dan hibernasi
	# tidak pernah jalan lagi. Bersihkan + pastikan app tidak nyangkut beku.
	if [ -f "$STATE" ]; then
...
status)
	...
	;;
watch)
	# Fallback watcher: hibernasi tetap jalan walaupun binary daemon yang
	# terpasang belum membawa kode screen-off ECO.
