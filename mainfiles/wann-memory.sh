#!/system/bin/sh
# Wann Optimizer fork - tuning memori (ZRAM + swappiness) untuk RAM kecil.
#
# Dipanggil service.sh saat boot: wann-memory.sh apply
# Manual / debug                : wann-memory.sh apply | status
#
# Config (/data/adb/.config/wann/mem/):
#   enabled     1/0   (default 1)
#   zram_mb     angka MB target ZRAM (default: 50% RAM, min 512, maks 4096)
